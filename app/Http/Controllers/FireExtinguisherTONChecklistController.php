...
use App\Models\FireExtinguisherTON;
use App\Models\FireExtinguisherTONChecklist;
use Carbon\Carbon;
use Illuminate\Http\Request;

class FireExtinguisherTONChecklistController extends Controller
{
    public function index(Request $request)
    {
        ...
        $fire_extinguisher_ton_id = $request->get('fire_extinguisher_ton_id', '1');

        // Base query with optional search
        $query = FireExtinguisherTONChecklist::where('fire_extinguisher_ton_id',$fire_extinguisher_ton_id)->with('fire_extinguisher_ton')
    ...
    {
        return response()->json([
            'fire_extinguisher_ton' => FireExtinguisherTON::all(),
        ]);
    }
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'fire_extinguisher_ton_id' => 'required|numeric|exists:fire_extinguisher_t_o_n_s,id',
        ...
        // Check if a record already exists for the same fire_extinguisher_ton_id and month
        $exists = FireExtinguisherTONChecklist::where('fire_extinguisher_ton_id', $validated['fire_extinguisher_ton_id'])
        ...
        $data = FireExtinguisherTONChecklist::create($validated);
    ...
    {
        try {
            $data = FireExtinguisherTONChecklist::with('fire_extinguisher_ton')->findOrFail($id);
            return response()->json([
                'data' => $data,
            ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        $record = FireExtinguisherTONChecklist::findOrFail($id);
    ...
    public function destroy($id){
        try {
            $data=  FireExtinguisherTONChecklist::findOrFail($id);
            $data->delete();
            return response()->json([
