...
class FireExtinguisherTEJChecklistController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = FireExtinguisherTEJChecklist::with('emergency_extinguisher_tej')
    ...
    {
        return response()->json([
            'fire_extinguisher' => FireExtiguisherTEJ::all(),
        ]);
    }
    ...
    {
        try {
            $data = FireExtinguisherTEJChecklist::with('emergency_exit_light')->findOrFail($id);
            return response()->json([
                'data' => $data,
            ]);
