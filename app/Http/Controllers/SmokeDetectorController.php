...
class SmokeDetectorController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'tag_no' => 'required|unique:smoke_detectors',
            'type' => 'required',
            'location' => 'required',
        ]);

        $data = SmokeDetector::create($validated);
        return response()->json([
    ...
    {
        try {
            $data = SmokeDetector::findOrFail($id);
            return response()->json([
                'data' => $data,
            ]);
    ...
    {
        $validated = $request->validate([
            'tag_no' => 'required|unique:smoke_detectors,tag_no,' . $id,
            'type' => 'required',
            'location' => 'required',
        ]);
        try {
